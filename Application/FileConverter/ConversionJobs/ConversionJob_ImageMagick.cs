...
namespace FileConverter.ConversionJobs
{
    ...
    public class ConversionJob_ImageMagick : ConversionJob
    {
        ...
        private static readonly object MagickInitLock = new object();
        private static bool magickInitialized;

        private bool isInputFilePdf;
        private int pageCount;

        static ConversionJob_ImageMagick()
        {
            // Process-level limits before any instance can read untrusted files.
            EnsureMagickSafeDefaults();
        }
        ...
        /// <summary>
        /// Applies resource limits once per process, before any Magick read (including page counting).
        /// </summary>
        public static void EnsureMagickSafeDefaults()
        {
            ...
            lock (MagickInitLock)
            {
                ...
                try
                {
                    // Soft caps; large enough for legitimate office/image work, low enough to reduce DoS risk.
                    ResourceLimits.Width = 20000;
                    ResourceLimits.Height = 20000;
                    ResourceLimits.Area = 20000L * 20000L;
                    ResourceLimits.Memory = 1024UL * 1024UL * 1024UL; // 1 GiB
                    ResourceLimits.Disk = 2UL * 1024UL * 1024UL * 1024UL; // 2 GiB
                    magickInitialized = true;
                }
                catch (Exception exception)
                {
                    Debug.Log($"Failed to apply ImageMagick resource limits: {exception.Message}");
                }
            }
        }
        ...
        protected override int GetOutputFilesCount()
        {
            ...
            if (System.IO.Path.GetExtension(this.InputFilePath).ToLowerInvariant() == ".pdf")
            {
                using (MagickImageCollection images = new MagickImageCollection())
                {
                    ...
                    // Ping avoids fully decoding every page just to count frames.
