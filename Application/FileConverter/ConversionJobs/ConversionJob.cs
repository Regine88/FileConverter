    ...
    public class ConversionJob : INotifyPropertyChanged
    {
        ...
        public ConversionState State
        {
            ...
            private set
            {
                ...
                this.NotifyCancelCommandChanged();
            }
        }

        private void NotifyCancelCommandChanged()
        {
            try
            {
                ...
                // BeginInvoke avoids deadlocks if the UI thread ever waits on conversion.
                ...
                {
                    app.Dispatcher.BeginInvoke(new Action(() => this.cancelCommand?.NotifyCanExecuteChanged()));
                }
            }
            catch
            {
                // UI may be shutting down; conversion state still updated.
            }
        }
        ...
        public void PrepareConversion(params string[] outputFilePaths)
        {
            ...
            string extension = Helpers.GetFileExtension(this.initialInputPath);
            if (string.IsNullOrEmpty(extension))
            {
                this.ConversionFailed(Properties.Resources.ErrorInputTypeIncompatibleWithOutputType);
                Debug.Log($"Input file has no extension: {this.initialInputPath}");
                return;
            }
        ...
        public void StartConversion()
        {
            ...
            // Result validation must happen before committing success so a missing output never reports Done.
            if (this.State == ConversionState.Failed)
            {
                this.OnConversionFailed();
            }
            else if (!this.AllOutputFilesExists())
            {
