...
namespace FileConverter
{
    ...
    public partial class Application : System.Windows.Application
    {
        ...
        protected override void OnExit(ExitEventArgs e)
        {
            ...
            if (!this.isSessionEnding && upgradeService.UpgradeVersionDescription != null && upgradeService.UpgradeVersionDescription.NeedToUpgrade)
            {
                ...
                else
                {
                    ...
                    while (upgradeService.UpgradeVersionDescription != null &&
                           upgradeService.UpgradeVersionDescription.InstallerDownloadInProgress)
                    {
                        Thread.Sleep(1000);
                    }

                    UpgradeVersionDescription upgradeDescription = upgradeService.UpgradeVersionDescription;
                    if (upgradeDescription == null || !upgradeDescription.NeedToUpgrade)
                    {
                        Debug.Log("Upgrade was cancelled or failed during download.");
                        return;
                    }

                    string installerPath = upgradeDescription.InstallerPath;
                    ...
                    // Integrity and publisher checks before executing the package.
                    if (!UpgradeService.VerifyInstallerPackage(upgradeDescription, out string verifyError))
                    {
                        Debug.LogError($"Refusing to run untrusted installer: {verifyError}");
                        return;
                    }

                    // Start process.
                    Debug.Log($"Start file converter upgrade from version {ApplicationVersion} to {upgradeDescription.LatestVersion}.");
                    upgradeDescription.State = UpgradeState.Installing;
                ...
                if (argument.StartsWith("--"))
                {
                    ...
                    switch (parameterTitle)
                    {
                        ...
                        case "input-files":
                            ...
                            {
                                // Delete immediately after successful read to avoid leaving user paths on disk.
                                try
                                {
                                ...
                                {
                                    // Extension Exited handler is a backup cleaner.
                                }
                            }
        ...
        private void RunConversions(List<string> filePaths, string conversionPresetName)
        {
            ...
            // Check for upgrade (fire-and-forget with exception observation).
            if (settingsService.Settings.CheckUpgradeAtStartup)
            {
                ...
                System.Threading.Tasks.Task upgradeTask = upgradeService.CheckForUpgrade();
                upgradeTask.ContinueWith(
                    t =>
                    {
                        if (t.IsFaulted && t.Exception != null)
                        {
                            Debug.Log($"Upgrade check failed: {t.Exception.GetBaseException().Message}");
                        }
                    },
            ...
            if (conversionPreset != null)
            {
                ...
                // Create conversion jobs. A single bad path must not abort the whole batch.
                ...
                for (int index = 0; index < filePaths.Count; index++)
                {
                    ...
                    {
                        Debug.LogError($"Failed to create conversion job for '{inputFilePath}': {exception.Message}");
                    }
                }

                if (conversionService.ConversionJobs.Count == 0)
                {
                    Debug.LogError("No conversion jobs could be created from the selected files.");
                    Application.AskForShutdown();
                    return;
                }
